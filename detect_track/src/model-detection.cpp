...
int rkYolov5s::init(rknn_context *ctx_in, bool share_weight)
{
    ...
    printf("api version: %s driver version: %s\n", version.api_version, version.drv_version);

    // Obtain the input and output parameters of the model
...
int NanoTrack_head::init(rknn_context *ctx_in, bool isChild) 
{
    printf("Loading model...\n");
    int model_data_size = 0;
    model_data = load_model(model_path.c_str(), &model_data_size);
    ...
    {
        printf("model is NCHW input fmt\n");
        channel0 = input_attrs[0].dims[1];
        height0 = input_attrs[0].dims[2];
        width0 = input_attrs[0].dims[3];
    }
    else
    {
        printf("model is NHWC input fmt\n");
        height0 = input_attrs[0].dims[1];
        width0 = input_attrs[0].dims[2];
        channel0 = input_attrs[0].dims[3];
    }

    if (input_attrs[1].fmt == RKNN_TENSOR_NCHW)
    {
        printf("model is NCHW input fmt\n");
        channel1 = input_attrs[1].dims[1];
        height1 = input_attrs[1].dims[2];
        width1 = input_attrs[1].dims[3];
    }
    else
    {
        printf("model is NHWC input fmt\n");
        height1 = input_attrs[1].dims[1];
        width1 = input_attrs[1].dims[2];
        channel1 = input_attrs[1].dims[3];
    }

    printf("model input0 height=%d, width=%d, channel=%d\n", height0, width0, channel0);
    printf("model input0 height=%d, width=%d, channel=%d\n", height1, width1, channel1);

    memset(inputs, 0, sizeof(rknn_input) * 2);
    inputs[0].index = 0;
    inputs[0].type = RKNN_TENSOR_UINT8;
    inputs[0].size = width0 * height0 * channel0;
    inputs[0].fmt = RKNN_TENSOR_NHWC;
    inputs[0].pass_through = 0;

    inputs[1].index = 0;
    inputs[1].type = RKNN_TENSOR_UINT8;
    inputs[1].size = width1 * height1 * channel1;
...
rknn_context *NanoTrack_head::get_pctx()
{
    return &ctx;
}

frameDets rkYolov5s::infer(cv::Mat &orig_img)
{
    ...
    // }
    frameDets dets(orig_img, detect_result_group);
