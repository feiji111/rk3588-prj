                 ...
                 std::vector<float> &qnt_scales, detect_result_group_t *group)
{
  ...
  for (int i = 0; i < validCount; ++i)
  {
    ...
    DetectBox detbox;
    detbox.x1 = (int)(clamp(x1, 0, model_in_w) / scale_w);
    detbox.y1 = (int)(clamp(y1, 0, model_in_h) / scale_h);
    detbox.x2 = (int)(clamp(x2, 0, model_in_w) / scale_w);
    detbox.y2 = (int)(clamp(y2, 0, model_in_h) / scale_h);
    detbox.box = cv::Rect_<float>(detbox.x1, detbox.y1, detbox.x2 - detbox.x1, detbox.y2 - detbox.y1);
    detbox.confidence = obj_conf;
    detbox.classID = id;
